...
```{r cov.mx, results='asis', eval=FALSE}
#scaled data
env1 <-round(var(scale(env)),2) #variance/covariance matrix
kable((env1), digits=2) #Formats table for output
```
...
require(MASS)
pca<-princomp(env,cor=TRUE) #run PCA on covariance and save all outputs in 'pca1'
summary(pca)
```
...
Proportion of Variation (PV):  Proportion of Variance is observed in the eigenvalues in the cononical matrix and is calculated with the following formula:

$PV=\frac{\lambda_i}{\lambda_i}$ where $i=1...p$ variables.  Equivalent to $R^2$ in regression.

Componenet 1 represents 26.3% of the variability along the primary axis, while  component 2 represents 23.8% of the variation along the secondary axis.  

**Eigenvectors of the PC axes**

```{r load, tidy=TRUE}
ld <-round(loadings(pca),2)
format(ld)
```
...
```{r Scree, fig.cap="Scree plot showing the variance (eigenvalues) for each principal component."}
